...
// nolint:interfacer
func (c *defaultLokiClient) Query(queryStr string, limit int, time time.Time, direction logproto.Direction, quiet bool) (*loghttp.QueryResponse, error) {
...
// excluding interfacer b/c it suggests taking the interface promql.Node instead of logproto.Direction b/c it happens to have a String() method
// nolint:interfacer
func (c *defaultLokiClient) QueryRange(queryStr string, limit int, from, through time.Time, direction logproto.Direction, step, interval time.Duration, quiet bool) (*loghttp.QueryResponse, error) {
...
func (c *defaultLokiClient) ListLabelNames(quiet bool, from, through time.Time) (*loghttp.LabelResponse, error) {
...
func (c *defaultLokiClient) ListLabelValues(name string, quiet bool, from, through time.Time) (*loghttp.LabelResponse, error) {
...
func (c *defaultLokiClient) Series(matchers []string, from, through time.Time, quiet bool) (*loghttp.SeriesResponse, error) {
	params := util.NewQueryStringBuilder()
	params.SetInt("start", from.UnixNano())
	params.SetInt("end", through.UnixNano())
...
func (c *defaultLokiClient) LiveTailQueryConn(queryStr string, delayFor int, limit int, from int64, quiet bool) (*websocket.Conn, error) {
	qsb := util.NewQueryStringBuilder()
	qsb.SetString("query", queryStr)
...
func (c *defaultLokiClient) GetOrgID() string {
	return c.OrgID
}

func (c *defaultLokiClient) doQuery(path string, query string, quiet bool) (*loghttp.QueryResponse, error) {
	var err error
...
func (c *defaultLokiClient) doRequest(path, query string, quiet bool, out interface{}) error {
...
func (c *defaultLokiClient) wsConnect(path, query string, quiet bool) (*websocket.Conn, error) {
	us, err := buildURL(c.Address, path, query)
	if err != nil {
		return nil, err
	}
